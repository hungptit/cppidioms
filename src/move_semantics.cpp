...
struct Base {
    ...
    Base(Base &&val) : name(std::move(val.name)) { std::cout << "Move constructor\n"; }
...
template <typename T> T create2(T &&val) { return T(std::forward<T>(val)); }

int myfunc(const int value = 0) {
    return value * 2;
}
