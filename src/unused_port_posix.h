...
class UnusedPort {
 public:
  static int port() {
    ...
    if (unused_port.port_ < 0)
      throw std::runtime_error("Cannot find a valid unused port.");
 ...
 private:
  ...
  int get_unused_port() {
    ...
    *static_cast<uint32_t*>(&addr.sin_addr.s_addr) = 0;
    ...
    int errcode =
        bind(fd_, reinterpret_cast<struct sockaddr*>(&addr), addr_len);
    if (errcode != 0) {
    ...
    errcode =
        getsockname(fd_, reinterpret_cast<struct sockaddr*>(&addr), &addr_len);
    if (errcode != 0) {
  ...
  // 1000 is a big number and the chance for us not to get a valid port number
  // is almost 0. If the random port number distribution is uniform.
